...
> ⚠️ **Superseded by ADR 0001 and ticket 02 build.** OpenF1 `date_start` matches
> f1api.dev `schedule.qualy.date` (Qualifying day), not `schedule.race.date`
> (race day). The implementation uses `qualyDate`; `race-date match` in this
> ticket is the wrong date field.
