...
Each section fails independently under the current ViewModel composition
(network-backed sections via `Flow.onStart{load()} + stateIn(SharingStarted.Lazily)`,
...
flowchart LR
  ...
  S1 -.-> UC1[GetSeason + GetNextRace + RaceSchedule conversion]
  S2 -.-> UC2[GetSeason]
  S3 -.-> UC3[GetNextRace + local CircuitArtwork]
```

## Section 1 — Next-race countdown

A single hero card presents the next race weekend: local circuit artwork,
next-session countdown, session status, local time, and GP name. Favorites
are rendered separately in §3.

- Data: `GetSeasonUseCase` + `GetNextRaceUseCase`; the matching
  `Race.schedule` is converted to typed `SessionTime` values. Artwork comes
  from `CircuitArtwork.forId` and is available offline.
...
- **Countdown card** (last card in the pager): a decorative circuit
  track-layout image (`Image` + `painterResource`, loaded from local F1DB
  artwork) on the right side, tinted with the circuit
  ...
  falls back to a neutral placeholder for unknown IDs. The card ticks every
  30s via a `LaunchedEffect` so the value
  ...
  card shows `…` / `—`. Schedule slot timestamps are parsed to
...
- The Glance Countdown widget (ticket 07) is **independent** of this
  ...
  Both pull from the same f1api.dev race schedule source.
...
A card for the next race: **round number, GP name, lap count, and corner
count**.

- Data: `GetNextRaceUseCase` (round, name, circuit).
- **Lap count + corner count** — inlined in `GetNextRace`'s race/circuit
  block (`laps: Int`, `circuit.corners: Int`). No extra call.
- Top speed is intentionally absent from v1. No remaining runtime source
  provides the same measurement semantics.

## Use-case surface after this

Ticket 03 named the network use cases; the countdown adds no network use case:

- `RaceSchedule.toWeekendSchedule()` — pure conversion from f1api.dev slots
  to `WeekendSchedule?`, dropping malformed slots and returning null for an
  empty schedule. `kotlinx.datetime.Instant` is used for typed start times.
...
| §1 countdown schedule | f1api.dev | `/current` schedule slots |
| §1 countdown circuit image | F1DB build-time import | local `drawable-nodpi` WebP |
| §2 aggregates | f1api.dev | `/current` (client-side sum) |
| §3 round/name/laps/corners | f1api.dev | `/current/next` |
| §3 top speed | Not shipped | no supported runtime replacement |

## ViewModel derived-section loading

`weekendSchedule` is derived after `loadSeason()` and `loadNextRace()` by
matching `round` and converting the matching `Race.schedule`. There is no
reactive observer because the two source atoms have one sequential writer.

## Open questions (not blockers)

- **Top speed remains absent from v1.** It is not replaced by an unsupported
  metric; a build-time import can be reconsidered only as a separate effort.
