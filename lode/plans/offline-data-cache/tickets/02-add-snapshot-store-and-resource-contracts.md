...
- [x] Resource keys cover current-season schedule, next race/session, standings, catalogs, session results, pitstops, circuit metadata, circuit most-wins, and Wikipedia summaries.
- [x] Snapshots record payload kind/version, fetch/stale timestamps, season metadata, and last-attempt metadata.
- [x] Store observation emits stable per-key values and avoids unnecessary churn at the resource seam.
- [x] Temp-file tests prove default state, atomic writes, migration behavior, and corruption recovery.
- [x] Room remains absent from implementation unless measured fallback tripwires appear.
